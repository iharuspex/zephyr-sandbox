...
#define DT_DRV_COMPAT sitronix_st7789v_spi
...
#ifdef CONFIG_ST7789V_SPI_RGB565
#define ST7789V_PIXEL_SIZE 2u
#else
#define ST7789V_PIXEL_SIZE 3u
#endif /* CONFIG_ST7789V_SPI_RGB565 */
                         ...
                         const void *buf)
{
    ...
    LOG_DBG("Writing %dx%d (x,h) @ %dx%d (x,y)", desc->width, desc->height, x, y);
    ...
    st7789v_transmit(dev, ST7789V_CMD_RAMWR, (void *)write_data_start, 
                                    ...
                                    const enum display_pixel_format pixel_format)
{
    ...
    LOG_ERR("Pixel format change not implemented yet");
    return -ENOTSUP;
}
                                ...
                                const enum display_orientation orientation)
{
    ...
    LOG_ERR("Changing display orientation not implemented yet");
    return -ENOTSUP;
}
...
static int st7789v_init(const struct device *dev)
{
    ...
    if (!device_is_ready(config->cmd_data_gpio.port)) {
        LOG_ERR("D/C GPIO device is not ready");
        return -ENODEV;
    }
...
static int st7789_pm_action(const struct device *dev,
